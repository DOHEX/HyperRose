...
//
// 导航壳参照 InstallerX Revived 的 MiuixNavWrapper 结构：
// 布局分流（紧凑/宽屏）、悬浮底栏模式矩阵与 Backdrop 采样层隔离。
 ...
 * 悬浮底栏渲染模式矩阵。
 *
 * 基础模糊是唯一采样门控：关闭时悬浮栏无任何玻璃效果；
 * 液态玻璃是悬浮栏在采样可用时的可选渲染风格。
 */
fun floatingBottomBarMode(
    blurEnabled: Boolean,
    liquidGlassEnabled: Boolean,
): FloatingBottomBarMode = when {
    !blurEnabled -> FloatingBottomBarMode.None
    liquidGlassEnabled -> FloatingBottomBarMode.LiquidGlass
    else -> FloatingBottomBarMode.Blur
}

/** 主页面 Pager 的同步状态：底栏只调用 animateToPage，滑动反向同步 selectedPage。 */
